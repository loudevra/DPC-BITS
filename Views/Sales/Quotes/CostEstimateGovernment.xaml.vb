...
#Region "1. Variables & Constants"
        ...
        Private _paginatedPages As New List(Of List(Of Integer)) ' Stores indices for each page
        ...
        ' Layout Constants (Safety Buffer Applied for Legal Size)
...
#Region "3. The Pagination Engine (Core Logic)"
        ...
        Private Sub RecalculatePagination()
            ...
            For Each group In _categoryGroups
                ...
                ' 1. Account for Header Height
                If Not String.IsNullOrWhiteSpace(group.CategoryName) Then
                    If currentHeight + CategoryHeaderHeight > PageMaxHeight Then
                        _paginatedPages.Add(New List(Of Integer)(pageIndices))
                        pageIndices.Clear()
                        currentHeight = CategoryHeaderHeight
                    Else
                        currentHeight += CategoryHeaderHeight
                    End If
                End If

                ' 2. Account for Item Heights
                For Each item In group.Items
                    Dim h As Double = CalculateItemHeight(item)

                    If currentHeight + h > PageMaxHeight Then
                        _paginatedPages.Add(New List(Of Integer)(pageIndices))
                        pageIndices.Clear()
                        currentHeight = h
                        pageIndices.Add(globalIndex)
                    Else
                        currentHeight += h
                        pageIndices.Add(globalIndex)
                    End If
                ...
                ' 3. Account for Subtotal Height
                If currentHeight + SubtotalRowHeight > PageMaxHeight Then
                    _paginatedPages.Add(New List(Of Integer)(pageIndices))
                    pageIndices.Clear()
                    currentHeight = SubtotalRowHeight
                Else
                    currentHeight += SubtotalRowHeight
                End If
            Next

            ' Add the final accumulated items
            If pageIndices.Count > 0 Then
                _paginatedPages.Add(pageIndices)
            End If

            ' 4. Footer Clipping Check
            ' If current height consumes the footer space (250px), force a new page.
            If currentHeight > (PageMaxHeight - FooterSectionHeight) Then
                _paginatedPages.Add(New List(Of Integer)) ' Empty page for footer
            End If
        ...
        Private Function CalculateItemHeight(item As Dictionary(Of String, String)) As Double
            ...
            If item.ContainsKey("Description") AndAlso Not String.IsNullOrWhiteSpace(item("Description")) Then
                ...
                Dim ft As New FormattedText(
                    ...
                    12,
                    Brushes.Black,
                    1.0) ' Force 1.0 DPI logic to avoid laptop scaling bugs

                ft.MaxTextWidth = 400
                h += ft.Height
                h += ReservedSpaceForDescription
            End If
            Return h
        End Function
        ...
        Private Sub LoadPage(index As Integer)
            ...
            Dim currentIndices = _paginatedPages(index)
            RebuildVisualList(currentIndices)

            dataGrid.ItemsSource = itemDataSource

            ' Visibility Logic for Footer
        ...
        Private Sub RebuildVisualList(indices As List(Of Integer))
            Dim allowedSet As New HashSet(Of Integer)(indices)
            Dim globalIdx As Integer = 0

            For Each group In _categoryGroups
                Dim groupItemsOnPage As New List(Of Dictionary(Of String, String))

                For Each item In group.Items
                    If allowedSet.Contains(globalIdx) Then
                        groupItemsOnPage.Add(item)
                    End If
                    globalIdx += 1
                Next

                If groupItemsOnPage.Count > 0 Then
                    If Not String.IsNullOrWhiteSpace(group.CategoryName) Then
                        itemDataSource.Add(New OrderItems With {
                            .Description = group.CategoryName.ToUpper(),
                            .IsCategoryHeader = True,
                            .ProductDescriptionVisibility = Visibility.Collapsed
                        })
                    End If

                    For Each item In groupItemsOnPage
                        itemDataSource.Add(CreateVisualItem(item))
                    Next

                    ' Show subtotal only if the last item of group is present
                    Dim lastGroupItem = group.Items.Last()
                    If groupItemsOnPage.Contains(lastGroupItem) Then
                        itemDataSource.Add(New OrderItems With {
                            .UnitPrice = "Subtotal:",
                            .LinePrice = $"₱ {group.Subtotal:N2}",
                            .IsSubtotalRow = True,
                            .ProductDescriptionVisibility = Visibility.Collapsed
                        })
                    End If
...
#Region "4. Item & Group Processing"
        Private Sub GroupItemsByCategory()
            ...
            Dim lastCatName As String = ""

            For Each item In allItems
                Dim catName As String = ""
                If item.ContainsKey("Category") Then catName = item("Category").ToString().Trim()

                If catName <> lastCatName OrElse currentGroup Is Nothing Then
