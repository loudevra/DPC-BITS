...
Namespace DPC.Data.Controllers
    Public Class ProductController
        ...
        Public Shared variationCount As Integer = 1
        Public Const MaxVariations As Integer = 2
        ...
        Public Shared Sub InsertNewProduct(Toggle As System.Windows.Controls.Primitives.ToggleButton, Checkbox As Controls.CheckBox,
            ...
            InsertProduct.InsertNewProduct(Toggle, Checkbox,
        ...
        Public Shared Sub InsertNonVariationProduct(conn As MySqlConnection, transaction As MySqlTransaction, productID As String,
            ...
            InsertProduct.InsertNonVariationProduct(conn, transaction, productID,
                                     Warehouse, SellingPrice, BuyingPrice,
                                     DefaultTax, DiscountRate,
                                     StockUnits, AlertQuantity,
                                     ValidDate, SerialNumbers, Checkbox)
        End Sub
        ...
        Public Shared Sub InsertVariationProduct(conn As MySqlConnection, transaction As MySqlTransaction, productID As String)
            InsertProduct.InsertVariationProduct(conn, transaction, productID)
        End Sub
        ...
        Public Shared Sub InsertSerialNumbersForProduct(conn As MySqlConnection, transaction As MySqlTransaction,
            ...
            InsertProduct.InsertSerialNumbersForProduct(conn, transaction,
        ...
        Public ChangeIcon As Boolean = False
        'Variation popup
        Public Shared Function CreateVariationPanel(variation As ProductVariation, MainVariationContainer As StackPanel) As StackPanel
            Return RenderProduct.CreateVariationPanel(variation, MainVariationContainer)
        End Function
        'Add variation panel to variation popup
        Public Shared Sub AddNewVariation(MainVariationContainer As StackPanel)
            RenderProduct.AddNewVariation(MainVariationContainer)
        End Sub
        'Add option button to variation panel
...
#Region "Variation UI Management"
        ...
        Public Shared Sub LoadVariationCombinations(variationsPanel As StackPanel, buttonClickHandler As RoutedEventHandler)
            ...
            Dim variations As List(Of ProductVariation) = _savedVariations
            ...
            If variations Is Nothing OrElse variations.Count = 0 Then
                ...
                VariationManager.SelectVariationCombination("Default Variation")
                Return
            End If
            ...
            If variations.Count = 1 Then
                ...
                If variation.Options IsNot Nothing AndAlso variation.Options.Count > 0 Then
                    ...
                    For Each opt As VariationOption In variation.Options
                        ...
                        ' Select the first option as current if none selected
                        If isFirst AndAlso String.IsNullOrEmpty(VariationManager.CurrentCombination) Then
                            VariationManager.SelectVariationCombination(opt.OptionName)
                        End If
           ...
           variation2.Options IsNot Nothing AndAlso variation2.Options.Count > 0 Then
                    ...
                    For Each option1 As VariationOption In variation1.Options
                        For Each option2 As VariationOption In variation2.Options
                            ...
                            ' Select the first combination as current if none selected
                            If isFirst AndAlso String.IsNullOrEmpty(VariationManager.CurrentCombination) Then
        ...
        Public Shared Sub UpdateVariationSelection(container As StackPanel, selectedButton As System.Windows.Controls.Button)
            ...
            ' Select this variation in the VariationManager
            If selectedButton IsNot Nothing AndAlso selectedButton.Tag IsNot Nothing Then
                Dim combinationName As String = selectedButton.Tag.ToString()
                VariationManager.SelectVariationCombination(combinationName)
            End If
        End Sub
#End Region
...
#End Region
        ...
        Public Shared Sub InitializeVariationCombinations()
            ' Clear existing combinations first
            VariationManager = New ProductVariationManager()

            ' Check if we have any saved variations
            If _savedVariations Is Nothing OrElse _savedVariations.Count = 0 Then
                ' No variations, just add a default
                VariationManager.AddVariationCombination("Default Variation")
                Return
            End If

            ' Generate combinations based on saved variations
            If _savedVariations.Count = 1 Then
                ' Single variation case
                Dim variation As ProductVariation = _savedVariations(0)
                For Each opt As VariationOption In variation.Options
                    VariationManager.AddVariationCombination(opt.OptionName)
                Next
            ElseIf _savedVariations.Count = 2 Then
                ' Two variations case
                Dim variation1 As ProductVariation = _savedVariations(0)
                Dim variation2 As ProductVariation = _savedVariations(1)
            ...
            ' Select the first combination as default if none selected
            If String.IsNullOrEmpty(VariationManager.CurrentCombination) AndAlso VariationManager.GetAllVariationData().Count > 0 Then
                VariationManager.SelectVariationCombination(VariationManager.GetAllVariationData().Keys.First())
            End If
        End Sub
